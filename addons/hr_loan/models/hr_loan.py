...
class hr_loan(models.Model):
	...
	ceo_email = fields.Char("CEO Email-Id")
	...
	def button_ceo_approve(self):
		...
		self.ceo_email = self.env.user.login
		...
		for loan in self:
			...
			self.send_emails_for_ceo_approve()
	...
	@api.multi
	def send_emails_for_ceo_approve(self):
	...
	# 	self.env['mail.template'].browse(template.id).send_mail(self.id)
		...
		employee = self.env.ref('hr_loan.hr_loan_ceo_to_employee_mail')
		manager = self.env.ref('hr_loan.hr_loan_ceo_to_manager_mail')
		# finance = self.env.ref('hr_loan.hr_loan_ceo_to_finance_mail')
