...
class MyController(http.Controller):
    ...
    def take_exam(self, exam_slug):
        ...
        exam_result = http.request.env['etq.result'].sudo().create({'exam_id':exam.id, 'token': token, 'state': 'incomplete', 'name':request.env.user.id})
        
        if exam.fill_mode == "random":

            questions = random.sample(exam.name.questions, exam.fill_mode_random_number)
	...
	for field_name, field_value in kwargs.items():
        ...
        if request.env.user.partner_id.name != 'Public user':
            exam_result.name = request.env.user.id
