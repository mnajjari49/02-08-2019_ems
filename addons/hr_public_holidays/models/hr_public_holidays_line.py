...
class HrPublicHolidaysLine(models.Model):
    ...
    # Developer1 Date Validation
    @api.constrains('date')
    @api.onchange('date')
    def line_id_validation(self):
        line_ids_dates = self.env['hr.holidays.public.line'].search([])
        fridays = self.env['hr.holidays'].all_fridays()

        for holidays in line_ids_dates:
            for friday in fridays:
                if friday == self.date:
                    raise UserError('Your Selected Date is Week off.')
            if holidays.date == self.date:
                raise UserError('Your Selected Date Already Existed.')
