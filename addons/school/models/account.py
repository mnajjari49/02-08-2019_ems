...
class AccountInvoiceRefund(models.TransientModel):
	...
	refund_per = fields.Integer('Percentage', required=True)
	@api.multi
	def invoice_refund(self):
		data_refund = self.read(['filter_refund'])[0]['filter_refund']
		return self.with_context(percentage=self.refund_per).compute_refund(data_refund)
...
class AccountInvoice(models.Model):
	...
	def _refund_cleanup_lines(self, lines):
		...
		"""
		print ("=============contecccccccccccccc",self._context)
		result = []
		for line in lines:
			values = {}
			for name, field in line._fields.iteritems():
				if name in MAGIC_COLUMNS:
					...
					print ("11111-----------------",line,line[name])
					values[name] = line[name]
				elif name == 'invoice_line_tax_ids':
					print ("==============line",line,line[name])
					values[name] = [(6, 0, line[name].ids)]
			print ("=============valuessssssssssss",values)
			multiply = values.get('price_unit') * self._context.get('percentage')
			per = multiply /100
			values['price_unit'] =  per
